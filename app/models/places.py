...
class Estado(db.Model):
	...
	relacion_municipio = db.relationship("Municipio", backref='estados', lazy=True, 
...
class Municipio(db.Model):
	...
	fk_estado = db.Column(db.Integer, db.ForeignKey('estados.id', ondelete="CASCADE"), nullable=False)
	
	#relacion_ciudad = db.relationship("Ciudad", backref='ciudades', lazy=True, 
	#	cascade="all, delete, update", passive_deletes=True)
	relacion_colonia = db.relationship("Colonia", backref='municipios', lazy=True, 
		cascade="all, delete", passive_deletes=True)
